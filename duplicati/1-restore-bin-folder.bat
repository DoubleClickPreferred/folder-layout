...
if "%input%" == "Y" (
   ...
   if not defined letter (
      ...
      "C:\Program Files\Duplicati 2\Duplicati.CommandLine.exe" restore "file://%letter%:\backup\\" "C:\home\static\bin\*" --backup-name="Everything but repositories" --dbpath="C:\home\duplicati\duplicati-db.sqlite" --passphrase=PASSPHRASE --disable-module=console-password-input --alternate-destination-marker=my_backup_is_here.txt --alternate-target-paths=*:\backup
   )
